...
# Gauges
model_load_time_seconds = Gauge("model_load_time_seconds", "Time taken to load the model")

model_info = Gauge("model_info", "Model metadata", ["tag", "saved_at"])
...
async def predict_price(request: PredictRequest):
    ...
    if model_pipeline is None:
        prediction_requests_total.labels(
            status="error", prefecture="unknown", property_type="unknown"
        ).inc()
    ...
    try:
        ...
        features = {
            ...
            "coverage_ratio": (
                request.coverage_ratio if request.coverage_ratio is not None else 60.0
            ),
            "floor_area_ratio": (
                request.floor_area_ratio if request.floor_area_ratio is not None else 200.0
            ),
        }
        ...
        prediction_requests_total.labels(
            status="error", prefecture=request.prefecture, property_type=request.property_type.value
        ).inc()
