...
def load_model_history():
    ...
    if "test_metrics" in df.columns:
        for metric in ["MAE", "RMSE", "MAPE"]:
            df[metric] = df["test_metrics"].apply(
...
def load_drift_reports():
    ...
    combined["report_ts"] = pd.to_datetime(
        combined["report_ts"], format="%Y%m%dT%H%M%SZ", errors="coerce"
    )

    # Group by report to get summary stats
    summary = (
        combined.groupby("report_ts")
        .agg({"value": ["mean", "max"], "status": lambda x: (x == "ALERT").sum()})
        .reset_index()
    )
    summary.columns = ["report_ts", "mean_drift", "max_drift", "num_alerts"]

    # Also keep full detail
    combined["numeric_alerts"] = combined.apply(
        lambda x: 1 if x["type"] == "numeric" and x["status"] == "ALERT" else 0, axis=1
    )
    combined["categorical_alerts"] = combined.apply(
        lambda x: 1 if x["type"] == "categorical" and x["status"] == "ALERT" else 0, axis=1
    )
...
def check_retraining_triggers(df_drift, df_history, df_pred, actuals):
    ...
    if not df_drift.empty:
        ...
        psi_violations = latest_drift[
            (latest_drift["metric"] == "PSI") & (latest_drift["value"] > PSI_THRESHOLD)
        ]
        jsd_violations = latest_drift[
            (latest_drift["metric"] == "JSD") & (latest_drift["value"] > JSD_THRESHOLD)
        ]
        ...
        if not merged.empty and "sale_price_yen" in merged.columns:
            ...
            if not df_history.empty:
                ...
                if current_mae > baseline_mae * MAE_DEGRADATION_THRESHOLD:
                    triggers.append(
                        f"🔴 MAE degraded: ¥{current_mae/1e6:.1f}M vs baseline ¥{baseline_mae/1e6:.1f}M"
                    )
    ...
    if not df_pred.empty and "ts_utc" in df_pred.columns:
        ...
        if len(daily_counts) > 1:
            ...
            if recent_avg > historical_avg * 2:
                triggers.append(
                    f"⚠️ Prediction volume spike: {recent_avg:.0f} vs {historical_avg:.0f} avg"
                )
...
def main():
    ...
    with col2:
        st.metric(
            "💾 Memory",
            f"{sys_metrics['memory_used_gb']:.1f}/{sys_metrics['memory_total_gb']:.0f}GB",
        )
    ...
    if triggers:
        ...
        st.button(
            ...
            type="primary",
            disabled=True,
            help="Would trigger CI/CD pipeline in production",
        )
    else:
        st.success("✅ All metrics within acceptable ranges. No retraining needed.")
        st.caption(
            f"Thresholds: PSI < {PSI_THRESHOLD}, JSD < {JSD_THRESHOLD}, MAE < {MAE_DEGRADATION_THRESHOLD}x baseline"
        )
    ...
    with col4:
        if not df_drift.empty:
            ...
            st.metric("Drift Alerts", total_alerts, delta="⚠️" if total_alerts > 0 else "✅")
        else:
    ...
    if feature_importance_df is not None:
        ...
        st.info(
            "💡 Feature importance not available. Train an XGBoost model for better explainability."
        )
    ...
    if not df_history.empty:
        ...
        ax.scatter(
            df_history["saved_at_utc"],
            df_history["MAE"] / 1e6,
            s=100,
            c=range(len(df_history)),
            cmap="viridis",
        )
    ...
    if not df_drift.empty:
        ...
        with col1:
            ...
            ax.set_ylabel("Feature")
            ax.axvline(
                PSI_THRESHOLD, color="orange", linestyle="--", label=f"Threshold ({PSI_THRESHOLD})"
            )
            ax.invert_yaxis()
            ax.legend()
            ax.grid(True, alpha=0.3, axis="x")
        ...
        st.dataframe(
            latest_drift[["feature", "type", "metric", "value", "status"]], use_container_width=True
        )
    else:
    ...
    st.markdown("---")
    st.caption(
        f"Dashboard refreshed at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Version: {get_git_commit()[:7]}"
    )
