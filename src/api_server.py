...
class PredictRequest(BaseModel):
    external_id: Optional[str] = Field(None, description="Client-provided identifier used later to match actual sale price")
    ...
    effective_area_m2: float = Field(..., gt=0, description="マンション=延床面積, 戸建て/土地=土地面積の近似でも可")
...
def _to_dataframe(req: PredictRequest) -> pd.DataFrame:
    ...
    df = pd.DataFrame([{
        ...
        "floor_area_ratio": req.floor_area_ratio if req.floor_area_ratio is not None else 200.0,
        "is_tokyo": is_tokyo,
    }])
